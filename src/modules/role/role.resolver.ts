...
export class RoleResolver {
  ...
  async paginatedRoles(
    ...
    const paginationResult =
      await this.roleService.listWithPagination(listInputDTO)
    return {
  ...
  @Query(() => Role)
  @Permissions('MasterApp:Role:Read')
  async findRoleById(@Args('id') id: number): Promise<Role> {
  ...
  async listTrashedRoles(
    ...
    const paginationResult =
      await this.roleService.listTrashedWithPagination(listInputDTO)
    return {
      data: paginationResult.data,
      meta: {
        totalItems: paginationResult.meta.totalItems,
